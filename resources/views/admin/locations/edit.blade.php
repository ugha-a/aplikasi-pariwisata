    ...
    <div class="container-fluid">
      <div class="row mb-2">
        <div class="col-sm-12 d-flex justify-content-between align-items-center">
          <div>
            <h1 class="m-0 font-weight-bold text-dark">Edit Lokasi</h1>
      ...
      <div class="card card-neo">
        <div class="card-header bg-white border-0 pb-0">
          <h5 class="mb-0 text-dark">
            <i class="fa fa-map-marker-alt mr-2 text-primary"></i> Form Edit Lokasi
          </h5>
        </div>
        ...
        <div class="card-body">
          <form method="POST" action="{{ route('admin.locations.update', $location) }}">
            ...
            <div class="form-row">
              ...
              {{-- Pengelola (opsional – aktifkan bila diperlukan)
              <div class="form-group col-md-6">
                ...
                <select id="user_id" name="user_id"
                  ...
                  @foreach($users as $u)
                    <option value="{{ $u->id }}"
                      {{ (string)old('user_id', $location->user_id) === (string)$u->id ? 'selected' : '' }}>
                      {{ $u->name }} — {{ $u->email }}
                    </option>
                  @endforeach
                </select>
...
<style>
  ...
  .btn-primary-blue:hover{
    background:#254ecf; border-color:#254ecf; transform:translateY(-1px);
  }

  /* CKEditor height */
  .ck-editor__editable_inline{ min-height:220px; }

  /* Select2 (jika diaktifkan) */
...
@section('scripts')
  ...
  <script>
    ...
    // Select2 (aktifkan jika field-nya dibuka)
    $('#user_id').select2({
      theme: 'bootstrap4',
      width: '100%',
