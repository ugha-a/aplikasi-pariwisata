    ...
    <div class="container-fluid">
      <div class="card card-neo">
        <div class="card-header bg-white border-0">
          <h5 class="mb-0 text-dark">Form Lokasi</h5>
        </div>
            ...
            <div class="form-row">
              ...
              {{-- <div class="form-group col-md-6">
                ...
                <select id="user" name="user"
                  ...
                  @foreach($users as $u)
                    <option value="{{ $u->id }}" {{ old('user') == $u->id ? 'selected' : '' }}>
                      {{ $u->name }} — {{ $u->email }}
                    </option>
                  @endforeach
                </select>
                @error('user') <small class="text-danger">{{ $message }}</small> @enderror
                <small class="form-text text-muted">Hubungkan lokasi dengan user (admin/dinas) yang bertanggung jawab.</small>
              </div>
            </div> --}}
...
@section('scripts')
  ...
  <!-- Select2 JS (butuh jQuery dari layout) -->
